...
ag.factory('SocketConnect', [
  'AppConfig', 'toast', 'GlobalLoading', '$rootScope', 'ConsoleMessages', function(app, toast, GlobalLoading, $scope, ConsoleMessages) {
    var socket;
    socket = io.connect("/");
...
ag.controller('ConsolePannelCtrl', [
  'AppConfig', '$scope', 'ConsoleMessages', function(app, $scope, ConsoleMessages) {
    $scope.ConsoleMessages = ConsoleMessages;
    $scope.$watch('ConsoleMessages.messages.length', function(n, o) {
      return $('.console-pannel-content').scrollTop(9999999);
    });
    $scope.y = 500;
    return $scope.move = function(x, y) {
      return $scope.$apply(function() {
        return $scope.y = y;
      });
    };
...
ag.controller('ProjectSidebarCtrl', [
  'AppConfig', '$scope', 'SocketConnect', 'ConsoleMessages', 'Schema', 'Requrl', 'Action', 'CodeFile', function(app, $scope, socket, ConsoleMessages, Schema, Requrl, Action, CodeFile) {
    $scope.app = app;
    $scope.Action = Action;
    $scope.Schema = Schema;
    $scope.Requrl = Requrl;
    ...
    return $scope.runAppItem = function() {
      app.getCurAppItemId().then(function(id) {
